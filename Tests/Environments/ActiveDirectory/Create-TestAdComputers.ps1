<#
.SYNOPSIS
    Creates test computer accounts in Active Directory for the YomiNet AD profile import feature.
    ...
    if ($PSCmdlet.ShouldProcess($name, "New-ADComputer in $OUPath")) {
        try {
            New-ADComputer `
                ...
                -Description "YomiNet AD import test ($(Get-Date -Format 'yyyy-MM-dd'))" `
                -Enabled $true `
                -ErrorAction Stop
...
Write-Host "Search Base for YomiNet: $OUPath" -ForegroundColor Yellow