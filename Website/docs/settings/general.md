---
sidebar_position: 0
description: "Configure which applications appear in the YomiNet sidebar and adjust general application behavior including thread pool settings."
keywords: [YomiNet, general settings, sidebar configuration, application settings, thread pool]
---
...
Type: `YomiNet.Models.ApplicationInfo`
...
The value 0 leaves the default settings (number of CPU threads). If the value is to high, performance problems may occur. If the value is higher than the max. threads of the ThreadPool, the max. threads will be used. Changes to this value will take effect after restarting the application. Wheter the value was set successfully can be seen in the log file under `%LocalAppData%\YomiNet\YomiNet.log`.
