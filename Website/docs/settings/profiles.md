---
sidebar_position: 9
description: "Configure where YomiNet stores profile files. Manage profile file locations for setup, portable, and custom deployment scenarios."
keywords: [YomiNet, profile settings, profile file location, profile storage, profile management]
---
...
| Version        | Path                                              |
| -------------- | ------------------------------------------------- |
| Setup / Archiv | `%UserProfile%\Documents\YomiNet\Profiles` |
...
- Path with environment variables (e.g., `%UserProfile%\\YomiNet\\Profiles`)
- UNC path (e.g., `\\\\server\\share$\\YomiNet\\Profiles`)
...
```json
{
  "Profiles_FolderLocation": "%UserProfile%\\YomiNet\\Profiles"
}
```
...
**Automatic backups**  
YomiNet automatically creates a backup of the profile file before applying any changes. See [Create daily backup](#create-daily-backup) and [Maximum number of backups](#maximum-number-of-backups) for configuration options.
...
**Type**: `List<YomiNet.Profiles.ProfileFileInfo>`
...
Profile files can be encrypted with a master password. See [FAQ > How to enable profile file encryption?](https://borntoberoot.net/YomiNet/docs/faq/profile-file-encryption#how-to-enable-profile-file-encryption) for more details.
