---
sidebar_position: 27
description: "View and manage IP-to-MAC address mappings for both IPv4 (ARP) and IPv6 (NDP) with YomiNet. Add or delete neighbor entries, filter by state, and inspect interface assignments."
keywords: [YomiNet, neighbor table, ARP table, NDP, IPv4, IPv6, ARP cache, NDP cache, IP to MAC, MAC address, address resolution, neighbor discovery]
---
...
Adding a static entry requires administrator privileges. Internally, YomiNet uses `New-NetNeighbor` (PowerShell `NetTCPIP` module) to create the entry with state `Permanent`.
