---
sidebar_position: 17
description: "Manage Windows Firewall rules with YomiNet. Add, edit, enable, disable, or delete inbound and outbound rules using a user-friendly interface."
keywords: [YomiNet, Windows Firewall, firewall rules, inbound, outbound, network security, firewall management]
---

# Firewall

The **Firewall** allows you to view, add, edit, enable, disable, or delete Windows Firewall rules managed by YomiNet. Rules are identified by a `YomiNet_` prefix in their display name so that only rules created through YomiNet are shown in this view.
...
The display name of every rule created with YomiNet is automatically prefixed with `YomiNet_` so it can be picked up by YomiNet on the next refresh. The prefix is hidden in the user interface.
...
The name is required and must not be empty. Internally the rule is stored with the prefix `YomiNet_` (e.g. `YomiNet_Webserver (HTTP/HTTPS)`).
...
**Type:** `YomiNet.Models.Firewall.FirewallRuleDirection`
...
**Type:** `YomiNet.Models.Firewall.FirewallRuleAction`
...
**Type:** `YomiNet.Models.Firewall.FirewallProtocol`
...
**Type:** `YomiNet.Models.Firewall.FirewallInterfaceType`
