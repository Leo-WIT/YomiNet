---
sidebar_position: 11
description: "AWS Session Manager integration for YomiNet (removed). Connect to EC2 instances using PowerShell with the AWS SSM plugin instead."
keywords: [YomiNet, AWS Session Manager, EC2, SSM plugin, remote session, AWS]
---
...
This feature has been removed! YomiNet **2025.10.18.0** will be the last version to support the **AWS Session Manager** feature.
...
You can still use AWS Session Manager within YomiNet with the [PowerShell](./powershell.md) feature by using the `aws ssm start-session --target <instance-id>` command.
...
This is an example of an IAM user policy to synchronize instances of AWS EC2 for YomiNet.
...
**Type:** `List<YomiNet.Models.AWS.AWSProfileInfo>`
