...
The integration of PowerShell with YomiNet supports tabs and profiles for hosts (and tools). You can launch the console / establish a connection via a profile (double-click, Enter key or right-click `Connect`) or directly via the [connection](#connect) dialog.
...
If Windows Terminal is installed, change the `Default terminal application` in the settings of Windows Terminal to `Windows Console Host` instead of `Windows Terminal` to use PowerShell in YomiNet.
...
**Type:** `YomiNet.Models.PowerShell.ExecutionPolicy`
