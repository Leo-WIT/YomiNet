...
**Groups and Profiles** let you organize hosts and networks for quick access across all YomiNet features. Settings defined in a group apply to every profile in that group; settings defined on a profile override the group defaults. See [FAQ > Settings priority](/docs/faq/settings-priority) for more information about the priority order.
