...
description: "How YomiNet fixes DPI scaling for embedded PowerShell and PuTTY windows in WPF using the Console API and WM_DPICHANGED."
...
This article documents the investigation and the two different solutions YomiNet uses for its embedded **PowerShell** (a console host process) and **PuTTY** (a GUI process) tabs — together with the complete, relevant C# source code.

<!-- truncate -->

## The Embedding Technique

YomiNet is a C#/WPF application that uses `WindowsFormsHost` to host a native Win32 `Panel` (WinForms `Panel`), and then calls `SetParent` to re-parent a foreign process window into that panel.
...
There is a subtle bug if the embedded process spawns on a **different monitor** than YomiNet: conhost's font is scaled for *its* monitor's DPI, not ours. Every subsequent `newDpi/oldDpi` relative rescale will then compound the error.
...
The full implementation is available in the YomiNet source:

- [`YomiNet.Utilities/NativeMethods.cs`](https://github.com/BornToBeRoot/YomiNet/blob/main/Source/YomiNet.Utilities/NativeMethods.cs) — all P/Invoke declarations and helpers
- [`YomiNet/Controls/PowerShellControl.xaml.cs`](https://github.com/BornToBeRoot/YomiNet/blob/main/Source/YomiNet/Controls/PowerShellControl.xaml.cs) — console host approach
- [`YomiNet/Controls/PuTTYControl.xaml.cs`](https://github.com/BornToBeRoot/YomiNet/blob/main/Source/YomiNet/Controls/PuTTYControl.xaml.cs) — GUI process approach
