...
description: "YomiNet now supports system-wide policies for centralized control over settings across all users — ideal for enterprise deployments."
authors: [borntoberoot]
tags: [policies, enterprise, settings, new feature]
keywords: [YomiNet, system-wide policies, enterprise, deployment, Intune, SCCM, Group Policy, configuration management]
---

YomiNet now supports system-wide policies, giving administrators centralized control over application settings across all users on a machine.
...
Policies are defined in a `config.json` file placed in the application installation directory (the same folder as `YomiNet.exe`). When this file exists, the application loads the policies at startup and applies them with precedence over user settings. An example file (`config.json.example`) is included in the application installation directory for reference.

**File location:**

- **Installed version**: `C:\Program Files\YomiNet\config.json` (or your custom installation path)
- **Portable version**: Same directory as `YomiNet.exe`
...
Property names follow the pattern `Section_SettingName`. You can find the available policy names and values in the corresponding [setting's documentation](https://borntoberoot.net/YomiNet/docs/category/settings).

### Example Policy

For example, the [`Update_CheckForUpdatesAtStartup`](https://borntoberoot.net/YomiNet/docs/settings/update) policy controls whether the application checks for new program versions on GitHub when the application is launched. The [`Profiles_FolderLocation`](https://borntoberoot.net/YomiNet/docs/settings/profiles) and [`Settings_FolderLocation`](https://borntoberoot.net/YomiNet/docs/settings/settings) policies allow administrators to redirect profile and settings storage to a centralized location, such as a network share or a custom local path.
...
- `Profiles_FolderLocation` / `Settings_FolderLocation`:
  ...
  - Path with environment variables (e.g., `%UserProfile%\\YomiNet\\Settings`)
  - UNC path (e.g., `\\\\Server\\Shared\\YomiNet\\Profiles`)
...
```json
{
  ...
  "Profiles_FolderLocation": "\\\\Server\\Shared\\YomiNet\\Profiles",
  "Settings_FolderLocation": "%UserProfile%\\YomiNet\\Settings"
}
```
...
2. **Deploy to installation directory** — Place the `config.json` file in the same directory as `YomiNet.exe`. For MSI installations, this is typically `C:\Program Files\YomiNet\`.
...
Additional policy options will be added in future releases to provide more granular control over application behavior. If you have specific requirements for system-wide policies in your organization, please submit a feature request via the [GitHub issue tracker](https://github.com/BornToBeRoot/YomiNet/issues/new/choose) to help us prioritize.

Upgrade now to the [latest version of YomiNet](https://borntoberoot.net/YomiNet/download) to use this feature.

More information is available in the [official documentation](https://borntoberoot.net/YomiNet/docs/system-wide-policies).
