...
description: "YomiNet now lets you import profiles directly from Active Directory. Query computer accounts via LDAP, review the results, and bulk-import them as profiles in seconds."
authors: [borntoberoot]
tags: [profiles, active directory, import, new feature]
keywords: [YomiNet, profile import, Active Directory, LDAP, bulk import, computer accounts, sysadmin]
---

YomiNet now lets you import profiles directly from **Active Directory**. Instead of creating profiles for every machine by hand, you can query your AD for computer accounts via LDAP, review the results, and bulk-import them into any group with a single click.

<!-- truncate -->

## The problem with large environments

If you manage dozens or hundreds of machines, maintaining YomiNet profiles manually is tedious. You have to add each host one by one, set the display name, pick the right group, and enable the applications you need. When machines are added or decommissioned in AD, keeping profiles in sync becomes its own maintenance burden.
...
Click **Search** and YomiNet queries the directory. All matching computer accounts appear in the next step.
...
- **New** — not yet in YomiNet, will be imported.
...
On the right side you choose which **applications** to enable for the imported profiles. [Ping Monitor](https://borntoberoot.net/YomiNet/docs/application/ping-monitor), [Remote Desktop](https://borntoberoot.net/YomiNet/docs/application/remote-desktop), and [PowerShell](https://borntoberoot.net/YomiNet/docs/application/powershell) are pre-selected, but you can change this before importing.
...
More information is available in the [official documentation](https://borntoberoot.net/YomiNet/docs/groups-and-profiles#import).
