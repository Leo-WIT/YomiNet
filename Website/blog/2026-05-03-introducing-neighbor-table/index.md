...
description: "YomiNet introduces the Neighbor Table, replacing the ARP Table with a unified view that covers both IPv4 (ARP) and IPv6 (NDP) address-to-MAC mappings."
authors: [borntoberoot]
tags: [neighbor table, arp, ndp, ipv6, refactoring]
keywords: [YomiNet, neighbor table, ARP table, ARP, NDP, IPv6, IPv4, MAC address, neighbor discovery, address resolution]
---

YomiNet introduces the new **Neighbor Table**, replacing the **ARP Table** with a unified view that covers IP-to-MAC address mappings for both IPv4 (ARP) and IPv6 (NDP) in a single place.
...
Rather than adding a separate IPv6 view, YomiNet now provides a single **Neighbor Table** that shows both IPv4 and IPv6 neighbors side by side, reflecting what the Windows networking stack actually maintains.
...
Click **Add entry...** below the table, supply an IPv4 or IPv6 address, the target MAC address, and the interface — YomiNet takes care of the rest using `New-NetNeighbor` under the hood.

:::note

Adding and deleting neighbor entries requires administrator privileges. If YomiNet is not running as administrator, the view is in read-only mode. Use the **Restart as administrator** button to relaunch with elevated rights.
...
More details are available in the [official documentation](https://borntoberoot.net/YomiNet/docs/application/neighbor-table).

If you find any issues or have suggestions, please open an [issue on GitHub](https://github.com/BornToBeRoot/YomiNet/issues).
