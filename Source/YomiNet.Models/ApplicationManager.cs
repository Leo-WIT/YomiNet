...
public static class ApplicationManager
{
    ...
    public static bool IsEnabled(ApplicationName name)
    {
        return name is not ApplicationName.IPGeolocation
            and not ApplicationName.RemoteDesktop
            and not ApplicationName.TigerVNC
            and not ApplicationName.WebConsole;
    }
...
#pragma warning disable CS0618
        return [.. GetNames().Where(x => IsEnabled(x) && x != ApplicationName.None && x != ApplicationName.AWSSessionManager && x != ApplicationName.ARPTable).Select(name => new ApplicationInfo(name, true, name == ApplicationName.Dashboard))];
