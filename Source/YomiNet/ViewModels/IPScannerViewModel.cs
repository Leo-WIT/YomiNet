...
public class IPScannerViewModel : ViewModelBase, IProfileManagerMinimal
{
    ...
    public async void OnLoaded()
    {
        ...
        if (string.IsNullOrWhiteSpace(Host))
            await TrySetDefaultLocalSubnetAsync();
    ...
    private async Task DetectSubnet()
    {
        ...
        var detectedSubnet = await DetectLocalSubnetAsync();

        if (!string.IsNullOrWhiteSpace(detectedSubnet))
        {
            Host = detectedSubnet;
    ...
    /// <summary>
    /// Sets the scan target to the local subnet when a new tab is created without an explicit target.
    /// </summary>
    private async Task TrySetDefaultLocalSubnetAsync()
    {
        IsSubnetDetectionRunning = true;
    ...
    /// <summary>
    /// Detects the local IPv4 subnet in CIDR notation based on the active route and network interface data.
    /// </summary>
